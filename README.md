...
<p align="center">
  ...
  <img alt="status: M1.0 done" src="https://img.shields.io/badge/status-M1.0%20done-green">
...
# 6. 想验证稳定性？连跑 5 次 M0 demo（约 2 分钟）
python tools/stability_run.py 5
```

第 5 步的 M1.0 demo 是**真正的并行编排**：会建 3 个 git worktree，2 个并行 worker
同时跑 Kiro CLI（带共享文件锁），最后串行 merge 回 main。如果 2 个 task 改了同
一文件的同一区域，会按设计停下交人工解冲突。
...
|------|------|--------------|------------|----------------------|--------|
| **kiro-conduit** | OSS, Python | **Kiro CLI** (ACP) | ✅ M1.0 (single-writer) | 计划 M1.1 | 计划 M2 |
...
- ✅ **唯一原生支持 Kiro ACP 协议**，可以直接驱动 `kiro-cli acp` 子进程（M0 / M1.0）
- ✅ **共享文件单一写者锁** + **拓扑波次并行调度** 已在 M1.0 端到端跑通
- 🟡 **接口锁定 stub-first** 是行业未明确做到的细分能力，规划 M1.1 落地（Augment Intent 部分实现，多数没做）
- 🟡 **跨仓库** M2 路线图
...
> 截至 2026-05-29 的 M1.0 实测（Kiro CLI Agent v2.4.2，本地 macOS）。

| 指标 | M0 PoC | M1.0 | 备注 |
|------|--------|------|------|
| 单元测试数量 | 49 | **102** | 27 DAG + 8 worktree + 7 locks + 5 orchestrator + 6 merge 等新增 |
| 集成测试数量 | 6 | 6 | mock ACP server，不依赖真 Kiro |
| 测试套件总耗时 | 1.93 s | **~7 s** | pytest 全量，仍在 CI 友好范围 |
| 源代码行数 | ~1100 | **~3000** | dag/worktree/locks/orchestrator/merge 5 个新模块 |
| 单任务 demo 成功率 | 5/5 (100%) | 5/5 (100%) | tools/stability_run.py 5 |
| 单任务 demo 平均耗时 | 25.2 s | 25.2 s | M0 demo 不变 |
| **多任务 DAG demo 成功率** | — | **3/3 task 全过** | examples/03_m1_demo.py |
| **多任务 DAG demo 耗时** | — | **~125 s** | 3 task: 1 串行 + 2 并行 |
| **共享文件锁成功仲裁** | — | **✓** | 实测日志显示 acquire/release 正确串行化 |
| **串行 merge 成功率** | — | **2/3** | pkg-sub 遇真实文本冲突按设计停下 |
| ruff 错误 | 0 | 0 | 已忽略中文标点警告（RUF001/002/003） |
| mypy strict 错误 | 0 | 0 | 全 15 个源文件 |
...
🚧 当前 **M1.0 done**（M1 MVP 第一阶段），正在准备 M1.1。

按 [docs/ROADMAP.md](docs/ROADMAP.md) 推进：

- [x] **M0：PoC** — 1 个 Coordinator + 1 个 Implementor + 1 个 Verifier 跑通最小链路（5/5 稳定）
- [x] **M1.0：核心骨架** — DAG 调度 + Git worktree 隔离 + 多 worker 并行 + 共享文件锁（single-writer）+ 串行 merge（端到端 demo 跑通）
- [ ] **M1.1：增强** — 接口锁定（stub-first）+ Verifier Layer 3（AI 语义 review）+ TUI dashboard + BYOA 模型路由 + append-only/coordinator-only 锁 policy
