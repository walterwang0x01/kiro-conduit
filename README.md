...
<p align="center">
  ...
  <img alt="status: M1.1 stub-first done" src="https://img.shields.io/badge/status-M1.1%20stub--first%20done-green">
  <img alt="tests" src="https://img.shields.io/badge/tests-148%20passed-brightgreen">
...
- ✅ **接口锁定 stub-first** 行业其他工具大多没显式做的细分能力，M1.1 step 1 落地：
  Verifier Layer 4 用 AST 抽签名 + diff，consumer 偷改接口直接拒（实测拦住了 LLM 的越界行为）
- 🟡 **跨仓库** M2 路线图
...
> 截至 2026-05-29 的 M1.1 step 1 实测（Kiro CLI Agent v2.4.2，本地 macOS）。

| 指标 | M0 PoC | M1.0 | M1.1 step 1 |
|------|--------|------|-------------|
| 单元测试数量 | 49 | 102 | **139** |
| 集成测试数量 | 6 | 6 | 6 |
| 测试套件总耗时 | 1.93 s | ~7 s | ~7 s |
| 源代码行数 | ~1100 | ~3000 | **~3300** |
| 单任务 demo 成功率 | 5/5 (100%) | 5/5 (100%) | — |
| **多任务 DAG demo 成功率** | — | 3/3 task | **4/4 task** |
| **多任务 DAG demo 耗时** | — | ~125 s | **~167 s**（4 task） |
| **共享文件锁成功仲裁** | — | ✓ | ✓ |
| **stub-first 接口锁定** | — | — | ✅ Layer 4 实测拦截 LLM 越界改 |
| **串行 merge 成功率** | — | 2/3 | **4/4** |
| ruff 错误 | 0 | 0 | 0 |
| mypy strict 错误 | 0 | 0 | 0 |
...
🚧 当前 **M1.1 stub-first 完成**（M1 MVP 第二阶段第一步），正在做 M1.1 剩余功能。
...
- [x] **M1.1 step 1：stub-first 接口锁定** — interface_lock + Verifier Layer 4 契约校验（4/4 task PASS + 4/4 merge 成功，M1.0 那个冲突彻底消除）
- [ ] **M1.1 step 2-4** — Verifier Layer 3（AI 语义 review）+ TUI dashboard + BYOA 模型路由 + append-only/coordinator-only 锁 policy
