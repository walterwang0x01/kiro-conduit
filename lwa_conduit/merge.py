...
- 每个分支：先在 task worktree 内 commit 改动 → 在一个独立的 integration worktree 里
  ...
  则把结果合到 lwa-conduit/integration 分支供人工 review）
...
from lwa_conduit.dag import Workspace, topological_waves
from lwa_conduit.git_utils import run_git
from lwa_conduit.worktree import WorktreeHandle
...
class MergeOrchestrator:
        ...
        async with self._git_lock:
            try:
                for tid in order:
                    ...
                    msg = commit_messages.get(tid, f"lwa-conduit: {tid}")
    ...
    def _publish_merge_started(self, task_id: str) -> None:
        if self._event_bus is None:
            return
        from lwa_conduit.events import MergeStarted
    ...
    def _publish_merge_finished(
        ...
        if self._event_bus is None:
            return
        from lwa_conduit.events import MergeFinished
    ...
    async def _ensure_integration(self, repo_path: Path, base_branch: str) -> Path:
        ...
        - 若 base_branch 正是主工作区当前分支（用户就坐在上面）：改用独立的
          `lwa-conduit/integration` 分支承载合并结果，base_branch 与主工作区全程不动，
          由用户事后 review 再合。
        """
        int_path = repo_path / ".lwa-conduit" / "integration"
        ...
        if current == base_branch:
            target = "lwa-conduit/integration"
