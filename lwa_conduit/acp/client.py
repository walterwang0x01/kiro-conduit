...
class AcpClientConfig:
    ...
    # lwa-conduit 默认全自动 allow_once，因为编排器跑在无人干预场景
...
class AcpClient:
    ...
    async def spawn(cls, config: AcpClientConfig | None = None) -> AcpClient:
        ...
        if cfg.sandbox_writable:
            from lwa_conduit.sandbox import wrap_command
    ...
    async def initialize(self) -> dict[str, Any]:
        ...
        result = await self._call(
            Method.INITIALIZE,
            {
                ...
                "clientCapabilities": {},
                "clientInfo": {"name": "lwa-conduit", "version": "0.0.1"},
            },
        )
    ...
    def _handle_response(self, msg: dict[str, Any]) -> None:
        msg_id = msg.get("id")
        # lwa-conduit 自己发的 request id 都是 int，反向请求的 id 是 str
