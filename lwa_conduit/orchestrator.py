...
from lwa_conduit.dag import TaskDef, Workspace, topological_waves
...
from lwa_conduit.locks import SharedFileLockManager
from lwa_conduit.memory import load_memory, memory_path, save_memory
from lwa_conduit.roles.coordinator import Coordinator, CoordinatorOutcome
...
from lwa_conduit.runtime.types import RuntimeConfig, coerce_runtime_kind
from lwa_conduit.types import Task
from lwa_conduit.worktree import WorktreeHandle, WorktreeManager

if TYPE_CHECKING:
    from collections.abc import Callable

    from lwa_conduit.semantic import SemanticReviewer
...
class ParallelOrchestrator:
    ...
    def _make_resumed_outcome(task_id: str, attempts: int) -> CoordinatorOutcome:
        ...
        from lwa_conduit.types import TaskResult, VerifyResult
    ...
    async def _commit_task(self, wt: WorktreeHandle) -> None:
        ...
        code, _, stderr = await run_git(
            wt.path,
            ["commit", "-m", f"lwa-conduit: {wt.task_id}"],
        )
        if code != 0:
            logger.warning(
    ...
    def _isolation_env(self, task_id: str) -> dict[str, str]:
        ...
        scratch = self._base_repo / ".lwa-conduit" / "scratch" / task_id
        scratch.mkdir(parents=True, exist_ok=True)
        return {
            "LWA_CONDUIT_TASK_ID": task_id,
            "LWA_CONDUIT_PORT_BASE": str(port_base),
            "LWA_CONDUIT_SCRATCH": str(scratch),
        }
    ...
    def _make_nudge_callback(
        ...
        from lwa_conduit.memory import make_retry_success_nudge
