...
from lwa_conduit.runtime.quota import probe_all_runtime_kinds, probe_runtime_kind
...
def test_native_kiro_usage_json(monkeypatch) -> None:
    from lwa_conduit.runtime import quota as quota_mod

    monkeypatch.delenv("LWA_CONDUIT_QUOTA_OVERRIDES", raising=False)
    quota_mod._CACHE.clear()
    payload = json.dumps({"remaining": 3, "limit": 10})
    proc = MagicMock(returncode=0, stdout=payload, stderr="")
    monkeypatch.setattr("lwa_conduit.runtime.native_quota.shutil.which", lambda _: "/bin/kiro-cli")
    with patch("lwa_conduit.runtime.native_quota.subprocess.run", return_value=proc):
